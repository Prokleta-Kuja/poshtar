...
import { type DomainLM, DomainService, type RelayLM, RelayService, type DomainVM, type RelayVM } from "@/api";
...
const domainData = reactive<{ params: IDomainParams, items: DomainLM[], delete?: DomainLM }>({ params: initParams(), items: [] });
const relayData = reactive<{ params: IDomainParams, items: RelayLM[], delete?: RelayLM }>({ params: initParams(), items: [] });
...
const refreshDomains = (params?: ITableParams) => {
    if (params)
        domainData.params = params;

    DomainService.getDomains({ ...domainData.params }).then(r => { domainData.items = r.items; updateParams(domainData.params, r) });
};
const refreshRelays = (params?: ITableParams) => {
    if (params)
        relayData.params = params;

    RelayService.getRelays({ ...relayData.params })
        .then(r => {
...
<template>
    <div class="d-flex align-items-center flex-wrap">
        ...
        <EditDomain v-if="update.domain" :relays="relayKv" :model="update.domain" :onUpdated="hideUpdateDomain" />
        <EditRelay v-if="update.relay" :model="update.relay" :onUpdated="hideUpdateRelay" />
    </div>
    <div class="d-flex flex-wrap">
        <Sizes class="me-3 mb-2" style="max-width:8rem" :params="domainData.params" :on-change="refreshDomains" />
        <Search autoFocus class="me-3 mb-2" style="max-width:16rem" placeholder="Name, Host"
            v-model="domainData.params.searchTerm" :on-change="refreshDomains" />
    </div>
    <div class="row">
        <div class="col-6">
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <Header :params="domainData.params" :on-sort="refreshDomains" column="relayName"
                                display="Relay" />
                            <Header :params="domainData.params" :on-sort="refreshDomains" column="disabled"
                                display="Disabled" />
                            <Header :params="domainData.params" :on-sort="refreshDomains" column="addressCount"
                                display="Addresses" />
                            <th></th>
                        </tr>
                    </thead>
                        ...
                        <tr v-for="item in domainData.items" :key="item.id" class="align-middle">
                            ...
                            <td class="text-end p-1">
                                <div class="btn-group" role="group">
                                    <button class="btn btn-sm btn-secondary" @click="showUpdateDomain(item)" title="Edit">
                                    ...
                                    <button class="btn btn-sm btn-danger" title="Delete" @click="showDeleteDomain(item)">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
            ...
            <ConfirmationModal v-if="domainData.delete" title="Domain deletion" :onClose="hideDeleteDomain"
                :onConfirm="deleteDomain" shown>
        ...
        <div class="col-6">
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <Header :params="relayData.params" :on-sort="refreshRelays" column="domainCount"
                                display="Domains" />
                            <th></th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="item in relayData.items" :key="item.id" class="align-middle">
                            ...
                            <td class="text-end p-1">
                                <div class="btn-group" role="group">
                                    <button class="btn btn-sm btn-secondary" @click="showUpdateRelay(item)" title="Edit">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                                            class="bi bi-pencil-square" viewBox="0 0 16 16">
                                            <path
                                                d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z" />
                                    ...
                                    <button class="btn btn-sm btn-danger" title="Delete" @click="showDeleteRelay(item)">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                                            class="bi bi-x-lg" viewBox="0 0 16 16">
                                            <path
                                                d="M2.146 2.854a.5.5 0 1 1 .708-.708L8 7.293l5.146-5.147a.5.5 0 0 1 .708.708L8.707 8l5.147 5.146a.5.5 0 0 1-.708.708L8 8.707l-5.146 5.147a.5.5 0 0 1-.708-.708L7.293 8 2.146 2.854Z" />
            ...
            <ConfirmationModal v-if="relayData.delete" title="Relay deletion" :onClose="hideDeleteRelay"
                :onConfirm="deleteRelay" shown>
