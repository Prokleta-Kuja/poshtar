...
const address = reactive<IModelState<AddressCM>>({ model: { domainId: 0, pattern: '', type: AddressType.Exact } })
...
const submit = () => {
    ...
    AddressService.createAddress({ requestBody: address.model })
        ...
        .catch(r => { address.error = r.body; address.submitting = false; })
}
        ...
        <template #body>
            <form @submit.prevent="submit">
                <TypeaheadBox class="mb-3" label="Domain" v-model="address.model.domainId"
                    :error="address.error?.errors?.domainId" :onSearch="searchDomain" required autoFocus />
