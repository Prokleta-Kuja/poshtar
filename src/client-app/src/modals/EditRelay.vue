    ...
    <Modal v-if="relay.model" title="Edit relay" shown :onClose="close">
        <template #body>
            <form @submit.prevent="submit">
                <Text class="mb-3" label="Relay" autoFocus v-model="relay.model.name" required
                    :error="relay.error?.errors?.name" />
                <CheckBox class="mb-3" label="Disabled" v-model="relay.model.disabled"
                    :error="relay.error?.errors?.disabled" />
                <Text class="mb-3" label="Host" :placeholder="'smtp.example.com'" v-model="relay.model.host" required
                ...
                <Text class="mb-3" label="Username" :autoComplete="'off'" v-model="relay.model.username" required
                    :error="relay.error?.errors?.username" />
                <Text class="mb-3" label="Replace password" :autoComplete="'off'" :type="'password'"
                    v-model="relay.model.newPassword" :error="relay.error?.errors?.newPassword" />
            </form>
        </template>
        <template #footer>
            <p v-if="relay.error" class="text-danger">{{ relay.error.message }}</p>
            <button class="btn btn-outline-danger" @click="close">Cancel</button>
            <SpinButton class="btn-primary" :loading="relay.submitting" text="Save" loadingText="Saving" @click="submit" />
        </template>
    </Modal>
</template>