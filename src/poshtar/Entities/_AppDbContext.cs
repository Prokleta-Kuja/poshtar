...
public partial class AppDbContext : DbContext, IDataProtectionKeyContext
{
    ...
    protected override void OnModelCreating(ModelBuilder builder)
    {
        ...
        {
            e.HasKey(e => e.CalendarEventId);
            e.HasOne(e => e.Calendar).WithMany(e => e.CalendarEvents).OnDelete(DeleteBehavior.Cascade);
        });
    ...
    public async ValueTask InitializeDefaults(IDataProtectionProvider dpProvider)
    {
        ...
        CalendarUsers.Add(new() { Calendar = icaAdminCalendar, User = icaAdmin });
        CalendarUsers.Add(new() { Calendar = icaUserCalendar, User = icaAdmin });
        CalendarUsers.Add(new() { Calendar = icaUserCalendar, User = icaUser });
        CalendarUsers.Add(new() { Calendar = nanAdminCalendar, User = nanAdmin });
        CalendarUsers.Add(new() { Calendar = nanUserCalendar, User = nanAdmin });
        CalendarUsers.Add(new() { Calendar = nanUserCalendar, User = nanUser });
